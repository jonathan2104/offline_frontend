import {Component, OnInit, Inject, ElementRef, ViewChild, Renderer2, AfterViewInit, Input, EventEmitter, Output} from '@angular/core';
...
export class Betslip2Component implements OnInit {
    ...
    constructor( private renderer: Renderer2, private soccerOddService: SocceroddsService, private tennisOddService: TennisoddsService, private matchService: MatchService, private tennisBetService: TennisBetsService, private sessionBetsService: SessionBetsService, private toastr: ToastrService, private exposureService: ExposureService, private matchBetService: MarketBetsService, private soccerBetService: SoccerBetsService, private userService: UserService, private bookMakerBetService: BookmakerBetsService, private FancyBetService: FancyBetsService, private soccerSessionBetService: SoccerSessionBetService, private soccerSessionService : SoccerSessionService, private tennisSessionBetService: TennisSessionBetService, private tennisSessionService : TennisSessionService) {
    }
    ...
    calculate() {
        if (this.data.m_type == 'session') {
            ...
            this.soccerBetService.getUserSoccerBetsByEventId(this.data.event_id,this.data.market_id).subscribe((res: any) => {
            ...
            this.soccerSessionBetService.getUserSoccerSessionBetsByEventId(this.data.event_id,this.data.market_id,this.data.m_type.trim()).subscribe((res: any) => {
                ...
                if (this.existing_soccerSessionBet.length == 0) {
                    ...
                    //console.log(this.existing_soccerBet[0].runner_name);
                    if (this.data.enable_draw) {
                        this.last_min_exp = Math.min(this.existing_soccerSessionBet[0].exp_amount1, this.existing_soccerSessionBet[0].exp_amount2, this.existing_soccerSessionBet[0].exp_amount3)
                    } else if (this.data.enable_draw == false) {
                        this.last_min_exp = Math.min(this.existing_soccerSessionBet[0].exp_amount1, this.existing_soccerSessionBet[0].exp_amount2)
                    }

                    if (this.existing_soccerSessionBet[0].runner_name == this.data.runner_name) {
            ...
            this.tennisSessionBetService.getUserTennisSessionBetsByEventId(this.data.event_id,this.data.market_id,this.data.m_type.trim()).subscribe((res: any) => {
                ...
                if (this.existing_tennisSessionBet.length == 0) {
                    ...
                    //console.log(this.existing_soccerBet[0].runner_name);
                    if (this.data.enable_draw) {
                        this.last_min_exp = Math.min(this.existing_tennisSessionBet[0].exp_amount1, this.existing_tennisSessionBet[0].exp_amount2, this.existing_tennisSessionBet[0].exp_amount3)
                    } else if (this.data.enable_draw == false) {
                        this.last_min_exp = Math.min(this.existing_tennisSessionBet[0].exp_amount1, this.existing_tennisSessionBet[0].exp_amount2)
                    }

                    if (this.existing_tennisSessionBet[0].runner_name == this.data.runner_name) {
        ...
        switch (index) {
            case 1:
                if (this.data.type == 'Back') {
                    this.finalProfit = +(this.price * this.count).toFixed(2) - this.count;
                    this.finalLoss = 0 - this.count;
                    this.exp_amount1 = this.existing_soccerSessionBet[0].exp_amount1 + this.finalProfit;
                    this.exp_amount2 = this.existing_soccerSessionBet[0].exp_amount2 + this.finalLoss;
                    this.exp_amount3 = this.existing_soccerSessionBet[0].exp_amount3 + this.finalLoss;
                } else if (this.data.type == 'Lay') {
                    this.finalProfit = this.count;
                    this.finalLoss = 0 - (+(this.price * this.count).toFixed(2) - this.count)
                    this.exp_amount1 = this.existing_soccerSessionBet[0].exp_amount1 + this.finalLoss;
                    this.exp_amount2 = this.existing_soccerSessionBet[0].exp_amount2 + this.finalProfit;
                    this.exp_amount3 = this.existing_soccerSessionBet[0].exp_amount3 + this.finalProfit;
                } else if (this.data.type == 'Draw') {
                    this.finalProfit = (+(this.price * this.count).toFixed(2)) - this.count;
                    this.finalLoss = 0 - this.count;
                    this.exp_amount1 = this.existing_soccerSessionBet[0].exp_amount1 + this.finalProfit;
        ...
        switch (index) {
            case 1:
                if (this.data.type == 'Back') {
                    this.finalProfit = +(this.price * this.count).toFixed(2) - this.count;
                    this.finalLoss = 0 - this.count;
                    this.exp_amount1 = this.existing_tennisSessionBet[0].exp_amount1 + this.finalProfit;
                    this.exp_amount2 = this.existing_tennisSessionBet[0].exp_amount2 + this.finalLoss;
                    this.exp_amount3 = this.existing_tennisSessionBet[0].exp_amount3 + this.finalLoss;
                } else if (this.data.type == 'Lay') {
                    this.finalProfit = this.count;
                    this.finalLoss = 0 - (+(this.price * this.count).toFixed(2) - this.count)
                    this.exp_amount1 = this.existing_tennisSessionBet[0].exp_amount1 + this.finalLoss;
                    this.exp_amount2 = this.existing_tennisSessionBet[0].exp_amount2 + this.finalProfit;
                    this.exp_amount3 = this.existing_tennisSessionBet[0].exp_amount3 + this.finalProfit;
                } else if (this.data.type == 'Draw') {
                    this.finalProfit = (+(this.price * this.count).toFixed(2)) - this.count;
                    this.finalLoss = 0 - this.count;
                    this.exp_amount1 = this.existing_tennisSessionBet[0].exp_amount1 + this.finalProfit;
                ...
                this.exposureService.getExposure().subscribe((data: any) => {
                    ...
                    if (this.data.m_type == 'session') {
                        ...
                        if (final_bal >= 0) {
                            ...
                            this.toastr.error("Insufficient Balance, you cannot place betkk.!");
                            // this.dialogRef.close();
                        }
                    } else if (this.data.m_type == 'bookmaker') {
                        this.submitted_disabled = true;
                        ...
                        if (final_bal >= 0) {
                                    ...
                                    this.soccerSessionService.CheckSoccerSessionChange(soccer_data).subscribe((res: any) => {
                                        {
                                            if (res.change) {
                                                this.toastr.error('Odds Changed..!!');
                                                // this.dialogRef.close();
                                            } else {
                                                this.soccerSessionBetService.addSoccerSessionBet(match_bet).subscribe((res: any) => {
                                                    if (res.hasOwnProperty('error')) {
                                                        this.toastr.error(res.error);
                                                        // this.dialogRef.close();
                                                    } else if (res.hasOwnProperty('message')) {
                        ...
                        if (final_bal >= 0) {
                                    ...
                                    this.tennisSessionService.CheckTennisSessionChange(tennis_data).subscribe((res: any) => {
                                        {
                                            if (res.change) {
                                                this.toastr.error('Odds Changed..!!');
                                                // this.dialogRef.close();
                                            } else {
                                                this.tennisSessionBetService.addTennisSessionBet(match_bet).subscribe((res: any) => {
                                                    if (res.hasOwnProperty('error')) {
                                                        this.toastr.error(res.error);
                                                        // this.dialogRef.close();
                                                    } else if (res.hasOwnProperty('message')) {
