...
export class CasinoDCardTreeTwoBPopupComponent implements OnInit {
  passwordForm: any;
  errorMsg: any;
  submitted = false;
  details: any = {};
  ...
  //   const [winner,oddeven,color,total] = this.extractArrayFromString(data.desc);
  ...
  private getResults(data: any) {
    ...
    const [player1,player2,player3,player4] = this.divideIntoArrays(data.cards);
    const [winner,oddeven,color,single,total,] = this.extractArrayFromString(data.desc);
    this.player1 = player1;
    this.player2 = player2;
    this.player3 = player3;
    this.player4 = player4;
    ...
    console.log(winner,oddeven,color,single,total);
