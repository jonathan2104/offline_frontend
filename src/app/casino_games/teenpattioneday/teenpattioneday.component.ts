...
@Component({
    selector: 'app-teen-teenpattioneday',
    templateUrl: './teenpattioneday.component.html',
    styleUrls: ['./teenpattioneday.component.css']
})
export class TeenPattiOnedayomponent implements OnInit, OnDestroy {
    ...
    getResultHistory() {
        this.resultSubscription = this.teen_pattiService.getResultHistoryOneDay().subscribe((res: any) => {
            if (res != null) {
    ...
    getLiveData() {
        this.dataSubscription = this.teen_pattiService.getLiveDataOneDay().subscribe((res: any) => {
            console.log('res-->', res);
            if (res != null) {
    ...
    getCasinoBetsByMarketId() {
        ...
        if (this.last_market_id != '') {
            this.betHistorySubscription = this.casinoBetsService.getCasinoBetsByMarketId(this.last_market_id, 'teen').subscribe((res: any) => {
    ...
    getCasinoExposureByMarketId() {
        ...
        if (this.last_market_id != '') {
            this.casinoExposureSubscription = this.casinoBetsService.getCasinoExposureByMarketId(this.last_market_id, 'teen').subscribe((res: any) => {
    ...
    openCasinoDetails(data:any){
        const dialogRef = this.dialog.open(CasinoDTennPattiOneDayPopupComponent, {
          autoFocus: false,
          maxHeight: '100vh',
          maxWidth: '100vh',
          width: '100vh',
