...
export class cashoutComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.subscription = this.cashoutService.calculate$.subscribe(() => {
            this.calculate();
          });
        ...
        if (this.data.m_type == 'session') {
            this.sessionBetsService.getSessionByRunnerEvent(this.data.event_id, this.data.runner_name.trim()).subscribe((res: any) => {
                ...
                if (this.existing_sessionBet.length == 0) {
                    ...
                    if (this.size <= 100) {
                        cashoutComponent.loss = 100;
                        cashoutComponent.profit = this.size;
                        this.finalLoss = cashoutComponent.loss;
                        this.finalProfit = cashoutComponent.profit;
                    } else {
                        cashoutComponent.loss = this.size;
                        cashoutComponent.profit = 100;
                        this.finalLoss = cashoutComponent.loss;
                        this.finalProfit = cashoutComponent.profit;
                    }
                    this.finalLoss = (this.count / 100) * cashoutComponent.loss;
                    this.finalProfit = (this.count / 100) * cashoutComponent.profit;
