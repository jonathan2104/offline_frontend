...
@Component({
    selector: 'app-casino-bet-slip',
    templateUrl: './casino-bet-slip.component.html',
    styleUrls: ['./casino-bet-slip.component.css']
})
export class CasinoBetSlipComponent implements OnInit {
    ...
    constructor(@Inject(MAT_DIALOG_DATA) public data: any, public dialogRef: MatDialogRef<any>, private toastr: ToastrService, private setButtonService: SetButtonService, private casinobetService: CasinoBetsService) {
    }
    ...
    submit() {
        ...
        switch (this.data.m_type) {
            ...
            case 'lucky7eu':
            case 'lucky7':
                let temp_runner_name = this.data.bet.nation.toLowerCase();
                temp_runner_name = temp_runner_name.split(' ').map((word: any) => word[0].toUpperCase() + word.substring(1)).join(' ');
                console.log('temp runner name lucky7',temp_runner_name)
                let lucky7_bet: any = {
                    user_id: this.data.user_id,
                    game_name: this.data.m_type,
                    g_type: 'casino',
            ...
            case 'dtl20':
                ...
                break;
            case 'card32':
                let card32a_runnername = this.data.bet.nat;
                let card32_bet: any = {
                    user_id: this.data.user_id,
                    game_name: this.data.m_type,
                    g_type: 'casino',
                    ...
                    s_id: this.data.bet.sid,
                    game_id: 10,
                    runner_name: card32a_runnername,
                    odds: this.data.bet.rate,
                    loss_amount: this.finalLoss,
                    win_amount: this.finalProfit,
                    index: this.data.bet.index,
            ...
            case 'card32eufancy':
            case 'card32eufancy1':
                let card32b_runnername = this.data.bet.nat;
                let card32eu_bet: any = {
                    ...
                    s_id: this.data.bet.sid,
                    game_id: 10,
                    runner_name: card32b_runnername,
                    odds: this.data.bet.rate,
                    loss_amount: this.finalLoss,
                    win_amount: this.finalProfit,
                    index: this.data.bet.index,
                ...
                break;
            case 'ab20':
                runnername = this.data.bet.nation;
                let ab20_bet: any = {
                    user_id: this.data.user_id,
                    game_name: this.data.m_type,
                    g_type: 'casino',
            ...
            case 'abj':
                ...
                console.log('abj_bet',abj_bet);
                this.casinobetService.addCasinoBet(abj_bet).subscribe((res: any) => {
                    if (res.error) {
                        this.toastr.error(res.error);
                    } else {
                        this.toastr.success(res.message);
