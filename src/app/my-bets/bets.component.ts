...
export class MybetListComponent implements OnInit {
  ...
  getAllBets() {
    forkJoin([
      this.matchBetService.getCloseBetsForMatch(180),
      this.bookmakerBetsService.getCloseBetsForBookmaker(180),
      this.sessionBetsService.getCloseBetsforSession(180),
      this.fancyBetsService.getCloseBetsforFancy(180),
      this.soccerBetService.getCloseBetsforSoccer(180),
      this.tennisBetsService.getCloseBetsforTennis(180),
    ]).subscribe({
      next: (responses: [any, any, any, any, any, any]) => {
        ...
        this.filter_bets = this.marge_bets;
        this.totalbets = this.filter_bets.length;
        this.totalAmount = this.filter_bets.reduce((sum:any, entry:any) => sum + (entry.bet_amount || 0), 0);
      },
      error: (error) => {
  ...
  filterBetsByType(event: any) {
    ...
    if (filterType === 'all') {
      ...
      this.filter_bets = this.marge_bets.filter((bet:any) => bet.from === "deleted");
    } else {
      this.filter_bets = this.marge_bets.filter((bet:any) => bet.type === filterType);
    }
    this.totalbets = this.filter_bets.length;
    this.totalAmount = this.filter_bets.reduce((sum:any, entry:any) => sum + (entry.bet_amount || 0), 0);
  }
